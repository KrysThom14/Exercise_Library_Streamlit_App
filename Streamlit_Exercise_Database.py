...
st.set_page_config(page_title='Exercise Database', 
                   page_icon='🏋️‍♂️', 
                   layout='wide')
...
if checkbox_2:
    ...
    with workout:
        workout = AgGrid(df_2, editable=True, sortable=True, filter=True, resizable=True, theme='balham')

    if st.button('Download Workout'):
        user_values = workout.data
        df_user = pd.DataFrame(user_values)
        csv_data = df_user.to_csv(index=False)
        b64 = base64.b64encode(csv_data.encode()).decode()
        href = f'<a href="data:file/csv;base64,{b64}" download="my_workout.csv">Download Workout</a>'
